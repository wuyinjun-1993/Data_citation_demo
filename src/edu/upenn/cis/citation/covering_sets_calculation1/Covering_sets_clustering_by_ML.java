...
public class Covering_sets_clustering_by_ML {
  ...
  public static void reasoning_single_tuple(ArrayList<HashSet<Covering_set>> head_variable_view_mapping, double[][] distances, HashSet<Covering_set> resulting_covering_sets, HashMap<String, Query> view_mapping_name_mappings) throws SQLException
  {
...
//      HashSet<citation_view_vector> curr_covering_sets = new HashSet<citation_view_vector>();
      ...
      for(int i = 0; i<points.size(); i++)
      {
        ...
        {
          all_covering_sets = (HashSet<Covering_set>) head_variable_view_mapping.get(points.get(i)).clone();
        }
        else
        {
          all_covering_sets = Join_covering_sets.join_operation(head_variable_view_mapping.get(points.get(i)), all_covering_sets);
        }
      }
