...
public class Aggregation4 {
	...
	public static void do_aggregate(ArrayList<Covering_set> curr_res, ArrayList<Covering_set> c_views, int seq, ArrayList<HashMap<String, HashSet<String>> > author_list, HashMap<String, HashMap<String, Integer>> view_query_mapping, HashMap<String, ArrayList<ArrayList<String>>> author_mapping, HashMap<String, Integer> max_num, IntList query_ids, ArrayList<Lambda_term[]> query_lambda_str, StringList view_list, HashMap<String, Boolean> full_flag, Connection c, PreparedStatement pst) throws ClassNotFoundException, SQLException
	{
		...
		if(seq == 0)
		{
			...
			{
				HashMap<String, HashSet<String>> json_citation = gen_citation0.join_covering_sets(c_views.get(i), c, pst, view_query_mapping, author_mapping, max_num, query_ids, query_lambda_str, view_author_mapping);
		...
		for(int i = 0; i<curr_res.size(); i++)
		{
			...
			{				
				...
				HashMap<String, HashSet<String>> authors = gen_citation0.join_covering_sets(c_vector, c, pst, view_query_mapping, author_mapping, max_num, query_ids, query_lambda_str, view_author_mapping);				
...
//				}
	...
	public static void do_aggregate_min(Covering_set c_views, int seq, HashMap<String, HashSet<String>> author_list, HashMap<String, HashMap<String, Integer>> view_query_mapping, HashMap<String, ArrayList<ArrayList<String>>> author_mapping, HashMap<String, Integer> max_num, IntList query_ids, ArrayList<Lambda_term[]> query_lambda_str, StringList view_list, HashMap<String, Boolean> full_flag, Connection c, PreparedStatement pst) throws ClassNotFoundException, SQLException
	{
...
//		}			
		...
		HashMap<String, HashSet<String>> authors = gen_citation0.join_covering_sets(c_views, c, pst, view_query_mapping, author_mapping, max_num, query_ids, query_lambda_str, view_author_mapping);				
...
//		Set<int[]> intervals = c_view_map.keySet();
		...
		int tuple_num = (tuple_level) ? Tuple_level_approach_min.tuple_num : Semi_schema_level_approach_min.tuple_num;
		...
		for(int i = 0; i<tuple_num; i++)
		{
			...
			if(tuple_level)
				Tuple_level_approach_min.get_views_parameters(single_views, rs, start_pos, lambda_values);
			else
				Semi_schema_level_approach_min.get_views_parameters(single_views, rs, start_pos, lambda_values);
...
//				if(view_mapping_string.contains(index_string))
	...
	static HashMap<String, HashSet<String>> gen_citation_view_level(citation_view single_view, HashSet<Head_strs> lambda_values, boolean tuple_level, Connection c, PreparedStatement pst) throws SQLException
	{
		...
		if(tuple_level)
		{
			citation_queries_ids = Tuple_level_approach_min.get_citation_queries(single_view.get_name());
			
			citation_queries = Tuple_level_approach_min.citation_queries;
			
			query_ids = Tuple_level_approach_min.query_ids;
		}
		else
		{
			citation_queries_ids = Semi_schema_level_approach_min.get_citation_queries(single_view.get_name());
			
			citation_queries = Semi_schema_level_approach_min.citation_queries;
			
			query_ids = Semi_schema_level_approach_min.query_ids;
		}
